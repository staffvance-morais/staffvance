    ...
    const fetchPerfil = async () => {
      try {
        ...
        if (session?.user) {
          ...
          if (roleNorm === 'admin' || roleNorm === 'owner' || roleNorm === 'coordenador' || roleNorm === 'producao' || roleNorm === 'produção') {
    ...
    <div className="min-h-screen bg-[#111111] font-sans flex flex-col items-center pb-20">
      <div className="w-full max-w-[800px] min-h-screen bg-[#1a1a1a] border-x border-[#333]">
        ...
        <div className="flex flex-col items-center pt-10 pb-6 border-b border-[#333]">
          ...
          <p className="text-[#2563eb] uppercase tracking-widest text-[12px] font-bold mt-1">{perfil?.cargo || (perfil?.role === 'producao' ? 'Produção' : perfil?.role) || role || 'staff'}</p>
        </div>
