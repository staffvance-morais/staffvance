...
import { Handshake, ChevronRight, ImagePlus, Delete, User, Building, Mail, Phone, FileEdit } from "lucide-react";

export default function CadastrarClienteCoordenador() {
  ...
  const [form, setForm] = useState({ representante: "", empresa: "", email: "", telefone: "", observacoes: "" });
  ...
  const handleCadastro = async () => {
    ...
    try {
      ...
      if (fotoArquivo) {
        ...
        const { error: uploadError } = await supabase.storage.from("clientes").upload(fileName, fotoArquivo);
        if (uploadError) throw uploadError;
        const { data: publicUrlData } = supabase.storage.from("clientes").getPublicUrl(fileName);
      ...
      router.push("/coordenador/clientes");
    } catch (error) {
    ...
    <div className="min-h-screen bg-[#0a0a0a] font-sans selection:bg-[#333] flex items-center justify-center p-4 md:p-8">
      <div className="w-full max-w-sm border border-[#333] bg-[#1c1c1c] relative flex flex-col min-h-[85vh]">
        <div className="p-5 pb-3">
          ...
          <p className="text-[#777] text-[13px] mb-4">Logo da empresa ou um retrato memoravel...</p>
          <div className="flex border border-[#333] bg-[#222] p-4 gap-4">
            <div className="w-28 h-28 bg-white shrink-0 relative overflow-hidden flex items-center justify-center">
              {comprimindo ? (<span className="text-xs text-gray-500 font-bold animate-pulse">Aguarde...</span>)
               : fotoPreview ? (<img src={fotoPreview} alt="Preview" className="w-full h-full object-cover" />) : null}
            </div>
            <div className="flex flex-col gap-2 justify-end">
              <button type="button" onClick={handleRemoverFoto} disabled={comprimindo || !fotoPreview}
              ...
              <button type="button" onClick={() => fileInputRef.current.click()} disabled={comprimindo || salvando}
        ...
        <div className="p-5 flex flex-col gap-3">
          ...
          <div className="flex items-start border border-[#333] bg-[#2a2a2a] p-3 focus-within:border-[#555] transition-colors h-28">
            ...
            <textarea name="observacoes" value={form.observacoes} onChange={handleChange} placeholder="Observacoes..." className="w-full h-full bg-transparent outline-none text-[#ddd] ml-3 placeholder:text-[#777] text-[15px] resize-none" />
          </div>
        </div>
        <div className="p-5 pt-2 flex flex-col gap-3 mt-auto">
          ...
          <button onClick={() => router.back()} disabled={salvando}
