...
function AlocarEquipeConteudo() {
  ...
  const handleSalvarEscala = async () => {
    ...
    try {
      ...
      // ATUALIZADO: Retorna para a tela de setores do Coordenador
      router.push(`/coordenador/eventos/${eventoId}/escalar`);
    } catch (error) {
      console.error(error);
      alert("Falha ao salvar a equipe. Verifique as permissões de banco de dados.");
    } finally {
      setSalvando(false);
    }
  };
