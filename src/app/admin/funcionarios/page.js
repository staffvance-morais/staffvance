...
import { Search, Info, Plus, Filter, ChevronUp, Menu, Users, User } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#141414] text-gray-300 font-sans flex flex-col relative pb-48">
      <div className="p-4 flex flex-col pt-6">
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="text-gray-300 text-base tracking-wide">Equipe Geral</span>
        </div>
      ...
      <div className="px-4 flex-1">
        <div className="relative flex items-center mb-2">
          ...
          <input type="text" placeholder="Toque para pesquisar" value={busca} onChange={(e) => setBusca(e.target.value)} className="w-full bg-[#262626] text-white outline-none py-3 pr-4 pl-10 border border-[#333] focus:border-[#555] transition-colors rounded-sm placeholder-gray-500" />
        </div>

        <p className="text-xs text-gray-400 font-medium mb-4">
          Listando {filtrados.length} de {equipeList.length} - <span className="font-bold text-gray-300">{selecionados.length} selecionados</span>
        </p>
            ...
            filtrados.map((membro) => {
              ...
              return (
                <div key={membro.id} className="relative border border-[#333] bg-[#1e1e1e] p-3 flex gap-3">
                  <div className="pt-1">
                    <div onClick={() => handleSelecionar(membro.id)} className={`w-5 h-5 border flex items-center justify-center cursor-pointer transition-colors ${selecionados.includes(membro.id) ? "bg-[#333] border-[#555]" : "border-[#444] bg-[#2a2a2a]"}`}>
                  ...
                  <div className="flex flex-col justify-center">
                    <h3 className="text-white font-bold text-lg leading-tight">{membro.nome_completo || "Nome não definido"}</h3>
                    <p className="text-gray-400 text-sm mt-1 capitalize">{membro.role || "Cargo não definido"}</p>
                  </div>

                  <button onClick={() => router.push(`/admin/funcionarios/${membro.id}`)} className="absolute right-3 bottom-3 w-8 h-8 border border-[#444] bg-[#2a2a2a] flex items-center justify-center text-gray-400 hover:bg-[#333] hover:text-white transition-colors rounded-sm cursor-pointer">
      ...
      <div className="fixed bottom-0 left-0 w-full bg-[#141414] p-4 flex flex-col gap-2 border-t border-[#222]">
        <button onClick={() => router.push("/admin/funcionarios/novo")} className="w-full bg-[#1e50cf] hover:bg-[#163a99] text-white font-medium text-lg py-4 flex items-center justify-center gap-2 rounded-sm transition-colors">
        ...
        <div className="w-full border border-[#333] p-3 mt-1 flex justify-between items-center bg-[#1a1a1a] rounded-sm">
          ...
          <button onClick={() => router.push("/admin")} className="p-2 border border-[#444] rounded-sm bg-transparent hover:bg-[#2a2a2a] transition-colors">
